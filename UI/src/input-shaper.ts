...
export const applyShaperToWelchPsdDb = (params: ShaperParams, psdDb: number[]) => {
  // PSD is power, so apply |H|^2.
  const out = new Array(psdDb.length);
  for (let i = 0; i < psdDb.length; i++) {
    const f = i * (FIXED_SAMPLE_RATE / (2 * (psdDb.length - 1)));
    const h = shaperMagnitudeAtHz(params, f);
    out[i] = psdDb[i] + 20 * Math.log10(Math.max(1e-12, h));
  }
  return out;
};
